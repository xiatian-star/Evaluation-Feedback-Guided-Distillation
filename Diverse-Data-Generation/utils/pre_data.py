...
def load_augmented_data(data_path, dataset = '', type_aug = '', size = 2):  # load the data to list(dict())
	...
	train_df = pd.read_csv(train_path, header=None, sep='\|+',names=['Index', 'Question','Equation'], index_col = False, dtype={'Index':str, 'Question':str, 'Equation':str})
...
def load_val_data(data_path, dataset, is_train = True): # load the data to list(dict())
	val_ls = None
	if is_train:
		train_path = os.path.join(data_path, dataset, 'val.csv')
		train_df = pd.read_csv(train_path)
		train_df['id'] = train_df.index
		val_ls = train_df.to_dict('records')
	return val_ls
...
def transfer_mathqa_num(train_ls, dev_ls, chall=False):
	print("Transfer numbers...")
	...
	for d in train_ls:
		...
		equation = " ".join(out_seq)
		out_seq = equation.replace('(','( ').replace(')',' )').replace('  ',' ').split()
		real_eq = from_infix_to_prefix((remove_brackets(out_seq)))
		temp_eq = []
		temp_idx = 1
		for ii in real_eq:
			if ii[0] == 'N':
		...
		train_pairs.append((input_seq, real_eq, nums, num_pos, 1))
	...
	for d in dev_ls:
		...
		for s in out_seq:  # tag the num which is generated
			...
			if s in generate_nums and s not in nums:
				generate_nums_dict[s] = generate_nums_dict[s] + 1
		equation = " ".join(out_seq)
		equation = equation.replace('(','( ').replace(')',' )').replace('  ',' ').split()
		real_eq = []
		for eq_idx in range(len(equation)):
			if equation[eq_idx] in ops or equation[eq_idx][0] == 'N' or equation[eq_idx] in generate_nums:
				if len(equation[eq_idx]) < 4:
		...
		assert len(nums) == len(num_pos)
        # pairs.append((input_seq, out_seq, nums, num_pos, d["ans"]))
		test_pairs.append((input_seq, real_eq, nums, num_pos, 1))
	...
	for g in generate_nums:
		if generate_nums_dict[g] >= 5:
			temp_g.append(g)
	#print(temp_g)
	return train_pairs, test_pairs, temp_g, copy_nums

def transfer_mathqaval_num(validation_pairs):
	pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
	ops = ['+','-','*','/','(',')','^']
	train_pairs = []
	generate_nums = []
	...
	for d in validation_pairs:
		...
		assert len(nums) == len(num_pos)
        # pairs.append((input_seq, out_seq, nums, num_pos, d["ans"]))
		train_pairs.append((input_seq, real_eq, nums, num_pos, 0))
